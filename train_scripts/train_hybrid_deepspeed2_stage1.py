...
trained_tokens = 0
avg_loss = 0
def on_train_batch_end_old(args, batch_idx, model_engine,teacher_engine, loss, teacher_loss, kl_loss, student_cross_entropy_loss, global_step, epoch, last_log_time, token_per_step, is_accumulation_step, pbar):
...
def on_train_batch_end(args, batch_idx, model_engine, teacher_engine, loss,
                       ...
                       is_accumulation_step, pbar,grad_norm=0):
    current_time = time.time()
    elapsed_time = current_time - last_log_time
    steps_per_second = 1 / elapsed_time
    kt_s = token_per_step * steps_per_second / 1000  # K tokens per second
    ...
    for epoch in range(args.max_epochs):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            if is_accumulation_step:
                ...
                # if grad_norm is not None and model_engine.global_rank == 0:
                #     print(f"[step {global_step}] grad_norm = {float(grad_norm):.4f}")
