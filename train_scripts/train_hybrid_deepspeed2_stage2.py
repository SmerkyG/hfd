...
trained_tokens = 0
avg_loss = 0
def on_train_batch_end_(args, batch_idx, model_engine,teacher_engine, loss, teacher_loss, kl_loss, student_cross_entropy_loss, global_step, epoch, last_log_time, token_per_step, is_accumulation_step, pbar):
    ...
    if real_step % args.save_per_batches == 0 and real_step > 0 :
        ...
        with teacher_attn_manager.temporarily_remove_teacher_attn():
            try:
                ...
                model_engine.save_checkpoint(output_dir, f'epoch_{epoch}_step_{real_step}',exclude_frozen_parameters=True)
