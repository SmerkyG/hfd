...
def radlads_replacement_attn(module, query, key, value, attention_mask, **kwargs):
...
def create_model_class(replacement_attention_class, base_model_path, base_attention_path, replacement_cache_layer_class=None):
    ...
    class RADLADSAttentionWrapper(base_model_attention_class):
        ...
        def forward(self, use_cache:bool, past_key_values:Optional[Cache], **kwargs):
            ...
            # FIXME - checkpointing didn't work here when I tried
            #student_output = torch_checkpoint(super().forward, use_cache=use_cache, past_key_values=past_key_values, **kwargs, use_reentrant=False)
            ...
            if self.teacher_attn is not None:
                ...
                with torch.no_grad():
                    teacher_output = self.teacher_attn.forward(use_cache=use_cache, past_key_values=past_key_values, **kwargs)
